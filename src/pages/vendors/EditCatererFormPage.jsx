...
const EditCatererFormPage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    customizableItems: [{ itemName: "", itemPrice: 0, itemType: "" }],
      ...
      try {
        ...
        const menuCategory =
          catererData.menuCategory?.length > 0
            ? catererData.menuCategory.map((cat) => ({
                ...cat,
                ...
                pricePerPlate: parseFloat(cat.pricePerPlate) || 0,
        ...
        setProfileImageFile(null); // Reset file state on load
      } catch (error) {
      ...
      menuCategory: prev.menuCategory.map((cat, i) =>
        i === index
          ? {
              ...cat,
              [field]:
                field === "pricePerPlate" ? parseFloat(value) || 0 : value,
            }
          : cat
      ),
    }));
      ...
      setFormData((prev) => ({
        ...
        customizableItems: [
          ...prev.customizableItems,
          {
            ...
            itemPrice: parseFloat(newCustomItem.itemPrice) || 0,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (
        ...
        toast.error(
          "Please fill all menu category details, add at least one menu item per category, and provide a valid price per plate."
        );
        return;
      }
